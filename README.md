...
Press **E** to open a nearby crystal (within 18 units, no aiming needed), or aim the ship at a signal up to 90 units away and fire with the same key. The centre reticle lights up and names the file that will open. Ranged shots show a bright tracer travelling from the ship to the crystal and an impact pulse before the viewer appears (about 0.6 seconds); nearby files still open instantly. Ranged shots use a 12° horizontal half-angle and allow up to 30 units of vertical offset; aim matters more than distance, and an atlas-selected file stays locked while within 90 units (nearby files still take priority). Use **M** to find a file and set a course. Space Drift pauses flight during the shot animation and while the file viewer is open: a ranged shot preserves your velocity for closing the viewer, while a nearby open or guided arrival holds position until a movement key resumes manual flight. Text/code, images, PDFs, audio, and video display inside the viewer. Press **Escape** or close the viewer to return to the same location.
